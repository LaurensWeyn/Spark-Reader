...
import com.lweyn.sparkreader.language.dictionary.*;
import com.lweyn.sparkreader.language.dictionary.JMDict.JMDictDefinition;
...
public class FoundDef implements Comparable<FoundDef>
{
    ...
    {
        defText = new TextBlockRenderer(Main.options.getOptionBool("defsShowUpwards"), width);
        //output source name TODO custom color for this
        defText.addText(foundDef.getSource().getName(), Main.options.getColor("defReadingCol"), Main.options.getColor("defBackCol"));

        //output original form if processed
        if(foundForm.getProcess().size() != 0)
        {
            defText.addText(foundForm.toString(), Main.options.getColor("defReadingCol"), Main.options.getColor("defBackCol"));
        }

        //output tags
        if(Main.options.getOptionBool("showAllTags") || !(foundDef instanceof JMDictDefinition))
            defText.addText(foundDef.getTagLine(), Main.options.getColor("defTagCol"), Main.options.getColor("defBackCol"));
        //output ID (debug)
        if(Main.options.getOptionBool("showDefID"))
            defText.addText(String.valueOf(foundDef.getID()), Main.options.getColor("defTagCol"), Main.options.getColor("defBackCol"));
        //output frequency data
        FrequencySink.FreqData freqdata = FrequencySink.get(this);
        if(freqdata != null)
            defText.addText(freqdata.toString(), Main.options.getColor("defTagCol"), Main.options.getColor("defBackCol"));

        //output readings
        for(Spelling reading:foundDef.getSpellings(foundForm))
        {
            if(reading.isKanji() && !Main.options.getOptionBool("showAllKanji"))continue; // note: showAllKanji currently broken
            //output readings if not in this form already
            if(!reading.getText().equals(foundForm.getWord()))
            {
                StringBuilder extraData = new StringBuilder("");
                //add frequency
                if(Main.options.getOptionBool("showReadingFreqs"))
                {
                ...
                if(Main.options.getOptionBool("showTagsOnReading") && reading.getTags() != null)
                {
                ...
                defText.addText(reading.getText() + extraData, Main.options.getColor("defReadingCol"), Main.options.getColor("defBackCol"));
            }
        }
        ...
        if(!(foundDef instanceof KanjiDefinition))
        {
            for (int i = 0; i < foundForm.getWord().length(); i++)
            {
                ...
                if (lookup != null)
                {
                    defText.addText(c + "【" + lookup + "】", Main.options.getColor("defKanjiCol"), Main.options.getColor("defBackCol"));
                }
            }
        }
        ...
        for(int i = 0; i < defs.size(); i++)
        {
            ...
            if(Main.options.getOptionBool("showTagsOnDef") && defs.get(i).getTags() != null)
            {
                ...
                defText.addText(startText + tags, Main.options.getColor("defTagCol"), Main.options.getColor("defBackCol"));
                startText = "";
            }
            if(foundDef instanceof JMDictDefinition && !Main.options.getOptionBool("splitDefLines"))
            {
                defText.addText(startText + defs.get(i).getMeaningAsLine(), Main.options.getColor("defCol"), Main.options.getColor("defBackCol"));
            }
            else
            {
                for(String defLine : defs.get(i).getMeaningLines())
                {
                    defText.addText(startText + defLine, Main.options.getColor("defCol"), Main.options.getColor("defBackCol"));
                    startText = "";
                }
            }
        }
