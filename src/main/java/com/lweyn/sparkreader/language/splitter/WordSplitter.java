...
public class WordSplitter
{
    ...
    private List<FoundWord> splitSection(String text, boolean firstSection)
    {
        ...
        while(start < text.length())
        {
            ...
            if(!Main.options.getOption("splitterMode").equals("none")) // (unless parsing is disabled)
            {
            ...
            while(pos > start)
            {
                ...
                if(matchedWord.getDefinitionCount() == 0 && Main.options.getOption("splitterMode").equals("full")) // (only if full parsing is enabled)
                {
                    matchedWord = null;
